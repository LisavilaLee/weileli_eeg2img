...
def evaluate_model(eegmodel, imgmodel, dataloader, device, text_features_all, img_features_all, k, save_dir, epoch, vae, image_processor):
    eegmodel.eval()
    total_loss = 0
    mae_loss_fn = nn.L1Loss()
    accuracy = 0
...
def main_train_loop(sub, current_time, eeg_model, img_model, train_dataloader, test_dataloader, optimizer, device,
                    ...
                    config, vae, image_preocessor, logger=None):
...
def main():
    ...
    for sub in subjects:
        ...
        if args.insubject:
            train_dataset = EEGDataset(data_path, subjects=[sub], train=True, device=device)
            test_dataset = EEGDataset(data_path, subjects=[sub], train=False, device=device)
        else:
            train_dataset = EEGDataset(data_path, exclude_subject=sub, train=True, device=device)
            test_dataset = EEGDataset(data_path, exclude_subject=sub, train=False, device=device)
        ...
        results = main_train_loop(sub, current_time, eeg_model, img_model, train_loader, test_loader, optimizer, device,
                                  ...
                                  config=args, vae=vae, image_preocessor=image_processor, logger=args.logger)
